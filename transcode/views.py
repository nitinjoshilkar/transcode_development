...
def transcode_data(request):
    ...
    try:
        ...
        transcode_data.response_formatter = [{  'end_point':'/api/transcode/update/job/status/','request_type':'POST','parameter':{'job_status':job_status,'job_id':job_id,'transcode_db_id':str(_id),'job_start_time':job_starttime,'job_end_time':job_endtime}}]
        ...
        return JsonResponse ({"message": "success", "data" : response_formatter_list, "status":200})
    except Exception as e:
        print(e)
        return Response({"error":"Invalid data"},status="400")
from rest_framework.permissions import IsAuthenticated
...
def transcode_job_update_status(request):
    ...
    return Response({"message": "updated","data":'OK'},status="200")
